...
content_ch = ['表盘模糊', '表盘破损', '外壳破损', '绝缘子破裂', '地面油污', '硅胶筒破损', '箱门闭合异常',
              '挂空悬浮物', '鸟巢', '盖板破损', '未穿安全帽', '未穿工装', '吸烟', '表计读数异常',
              '呼吸器油封油位异常', '硅胶表色', '压板合']
...
DR_PATH = os.path.join(os.getcwd(), 'input', 'ground-detect')
...
with open(results_files_path + "/results.txt", 'w') as results_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        text = content_ch[int(class_name)-1] + ": AP = {0:.2f}%".format(ap*100)
        ...
        if not args.quiet:
            if(len(rec)!=0):
                print(text + ' || score_threhold={} : '.format(MINOVERLAP) + "F1=" + "{0:.2f}%".format(F1[score05_idx]*100) \
